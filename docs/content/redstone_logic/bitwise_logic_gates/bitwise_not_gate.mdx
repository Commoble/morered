...
Bitwise logic plates can connect to [Bundled Cables](@morered:bundled_cable), [Colored Cables](@morered:orange_cable), or similar blocks. For each color channel, if the Bitwise NOT Gate is not receiving power from its input side on that channel, the output side will output a full-strength signal on that channel.
