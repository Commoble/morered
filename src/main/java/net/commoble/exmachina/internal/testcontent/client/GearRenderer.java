...
import net.commoble.exmachina.internal.testcontent.GearBlock;
...
public record GearRenderer(ItemRenderer itemRenderer, Map<Block, ItemStack> stackCache) implements BlockEntityRenderer<GenericBlockEntity>
{
	
	public static GearRenderer create(BlockEntityRendererProvider.Context context)
	{
		return new GearRenderer(context.getItemRenderer(), new HashMap<>());
	}
	...
	public void render(GenericBlockEntity be, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int overlay)
	{
		...
		{
			if (state.getValue(GearBlock.PROPERTY_BY_DIRECTION.get(facing)))
			{
