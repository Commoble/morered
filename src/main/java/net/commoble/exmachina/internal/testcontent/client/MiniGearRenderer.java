package net.commoble.exmachina.internal.testcontent.client;
...
import net.commoble.exmachina.internal.testcontent.GenericBlockEntity;
import net.commoble.exmachina.internal.testcontent.MiniGearBlock;
...
public record MiniGearRenderer(ItemRenderer itemRenderer, Map<Block, ItemStack> stackCache) implements BlockEntityRenderer<GenericBlockEntity>
{
	
	public static MiniGearRenderer create(BlockEntityRendererProvider.Context context)
	{
		return new MiniGearRenderer(context.getItemRenderer(), new HashMap<>());
	}
	...
	public void render(GenericBlockEntity be, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int overlay)
	{
		...
		Direction facing = state.getValue(MiniGearBlock.FACING);
		...
		long gameTimeTicks = level.getGameTime() % 6030;
		...
		poseStack.scale(0.33F, 0.33F, 1F);
