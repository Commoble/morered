...
public class WireSpoolItem extends Item
{
	...
	private ActionResultType onUseOnPost(World world, BlockPos pos, @Nonnull WirePostTileEntity post, ItemStack stack, PlayerEntity player)
	{
		...
		{
			CompoundNBT nbt = stack.getTagElement(LAST_POST_POS);
			
			if (nbt == null)
			{
				stack.addTagElement(LAST_POST_POS, NBTUtil.writeBlockPos(pos));
			}
				...
				else // we clicked a different post that doesn't have an existing connection to the original post
				{
					...
					if (hit != null)
					{
						...
						if (player instanceof ServerPlayerEntity && world instanceof ServerWorld)
						{
							...
							((ServerWorld)world).sendParticles((ServerPlayerEntity)player, RedstoneParticleData.REDSTONE, false, hit.x, hit.y, hit.z, 5, .05, .05, .05, 0);
							
							((ServerPlayerEntity)player).playNotifySound(SoundEvents.WANDERING_TRADER_HURT, SoundCategory.BLOCKS, 0.5F, 2F);
						}
					}
					// if post wasn't connected, connect them if they're close enough
					else if (pos.closerThan(lastPos, ServerConfig.INSTANCE.max_wire_post_connection_range.get()))
					{
						stack.removeTagKey(LAST_POST_POS);
						WirePostTileEntity.getPost(world, lastPos)
							.ifPresent(lastPost -> WirePostTileEntity.addConnection(world, post, lastPost));
						stack.hurtAndBreak(1, player, thePlayer -> thePlayer.broadcastBreakEvent(EquipmentSlotType.MAINHAND));
			...
			world.playSound(null, pos, SoundEvents.STONE_BUTTON_CLICK_ON, SoundCategory.BLOCKS,
	...
	public boolean shouldRemoveConnection(BlockPos connectionPos, World world, Entity holder)
	{
		...
		if (holder.position().distanceToSqr(Vector3d.atCenterOf(connectionPos)) > maxDistance*maxDistance)
		{
			return true;
		}
		...
		if (holder instanceof ServerPlayerEntity)
		{
			MoreRed.CHANNEL.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity)holder),
				new WireBreakPacket(
					...
					new Vector3d(holder.getX(), holder.getEyeY(), holder.getZ())));
		}
	}
}
