...
public class SlackInterpolator {
     ...
     * @param postPos              the position of the post block whose connections we want to check the
     *                             intersections of
     ...
     *                             Any posts in this list that this post is connected to is also connected to this
     *                             post, and this connection has been
     ...
     */
    @Nullable
    public static Vec3 doesBlockStateIntersectAnyWireOfPost(BlockGetter world, BlockPos postPos, BlockPos placePos,
                                                            BlockState placeState,
                                                            Map<BlockPos, NestedBoundingBox> remoteConnections,
                                                            Set<BlockPos> checkedPostPositions) {
        for (Entry<BlockPos, NestedBoundingBox> entry : remoteConnections.entrySet()) {
            ...
            if (!checkedPostPositions.contains(connectedPos)) {
                Vec3 hit = SlackInterpolator.doesBlockStateIntersectConnection(postPos, connectedPos, placePos,
                        placeState, entry.getValue(), world);
                if (hit != null) {
                    return hit;
                }
            }
    ...
    @Nullable
    public static Vec3 doesBlockStateIntersectConnection(BlockPos startPos, BlockPos endPos, BlockPos placePos,
                                                         BlockState placeState, NestedBoundingBox box,
                                                         BlockGetter world) {
    ...
    public static Vec3 getWireRaytraceHit(BlockPos lower, BlockPos upper, BlockGetter world) {
        Vec3 startVec = WirePostTileEntity.getConnectionVector(lower);
        Vec3 endVec = WirePostTileEntity.getConnectionVector(upper);
        ...
        for (int i = 0; i < rayTraceCount; i++) {
            WireRayTraceContext context = new WireRayTraceContext(selector, points[i], points[i + 1],
                    ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE);
            HitResult result = rayTraceBlocks(world, context);
...
//			IFluidState fluidState = world.getFluidState(pos);
            Vec3 startVec = rayTraceContext.getStartVec();
            Vec3 endVec = rayTraceContext.getEndVec();
...
//			double resultDistance = result == null ? Double.MAX_VALUE : rayTraceContext.getStartVec().squareDistanceTo
//			(result.getHitVec());
//			double fluidDistance = fluidResult == null ? Double.MAX_VALUE : rayTraceContext.getStartVec()
//			.squareDistanceTo(fluidResult.getHitVec());
            return result;
        }, (rayTraceContext) -> {
            Vec3 difference = rayTraceContext.getStartVec().subtract(rayTraceContext.getEndVec());
            return BlockHitResult.miss(rayTraceContext.getEndVec(), Direction.getNearest(difference.x, difference.y,
                    difference.z), new BlockPos(rayTraceContext.getEndVec()));
        });
    }

    static <T> T doRayTrace(WireRayTraceContext context, BiFunction<WireRayTraceContext, BlockPos, T> rayTracer,
                            Function<WireRayTraceContext, T> missFactory) {
        Vec3 start = context.getStartVec();
        Vec3 end = context.getEndVec();
        if (start.equals(end)) {
