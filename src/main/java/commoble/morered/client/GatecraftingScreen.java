...
public class GatecraftingScreen extends AbstractContainerScreen<GatecraftingContainer> {
    public static final ResourceLocation TRADING_SCREEN = new ResourceLocation("minecraft:textures/gui/container" +
            "/villager2.png");
    public static final ResourceLocation CRAFTING_SCREEN = new ResourceLocation("minecraft:textures/gui/container" +
            "/crafting_table.png");
    ...
    public GatecraftingScreen(GatecraftingContainer screenContainer, Inventory inv, Component titleIn) {
        super(screenContainer, inv, titleIn);
        this.imageWidth = 276;
        this.imageHeight = 166;
        this.name = new TranslatableComponent(BlockRegistrar.GATECRAFTING_PLINTH.get().getDescriptionId()).getString();
    }
    ...
    public void init() {
        ...
        List<Recipe<CraftingContainer>> recipes = world != null ?
                RecipeRegistrar.getAllGatecraftingRecipes(world.getRecipeManager()) : ImmutableList.of();
        this.scrollPanel = new GatecraftingScrollPanel(this.minecraft, this, recipes, xStart + SCROLLPANEL_X,
                yStart + SCROLLPANEL_Y, SCROLLPANEL_WIDTH, SCROLLPANEL_HEIGHT);
    ...
    protected void renderLabels(PoseStack matrix, int mouseX, int mouseY) {
        this.font.draw(matrix, this.name, this.imageWidth / 2 - this.font.width(this.name) / 2, 6, 4210752);    //
        // y-value and color from dispenser, etc
        this.font.draw(matrix, this.playerInventoryTitle, 107, this.imageHeight - 96 + 2, 4210752);
    }
...
//		this.minecraft.getTextureManager().bind(TRADING_SCREEN);
        ...
        GuiComponent.blit(matrix, arrowScreenX, arrowScreenY, blitWidth, arrowHeight, arrowU, arrowV, arrowWidth,
                arrowHeight, 512, 256);
    }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        ...
        return (this.scrollPanel.mouseDragged(mouseX, mouseY, button, deltaX, deltaY) || super.mouseDragged(mouseX,
                mouseY, button, deltaX, deltaY));
    }
    ...
    public static class RecipeButton extends ExtendedButton {
        ...
        public RecipeButton(GatecraftingScreen screen, Recipe<CraftingContainer> recipe, int x, int y, int width) {
            super(x, y, width, getHeightForRecipe(recipe), new TextComponent(""),
                    button -> onButtonClicked(screen.menu, recipe));
            this.baseY = y;
            this.screen = screen;
            this.recipe = recipe;
        }

        public static void onButtonClicked(GatecraftingContainer container, Recipe<CraftingContainer> recipe) {
        ...
        public void renderButton(PoseStack matrix, int mouseX, int mouseY, float partial) {
            ...
            if (this.visible) {
                ...
                if (ingredientCount > 0) {
                    ...
                    int extraIngredientRows = (ingredientCount - 1) / 3; //0 if 3 ingredients, 1 if 4-6 ingredients,
                    // 2 if 7-9 ingredients, etc
...
//		            this.screen.minecraft.textureManager.bind(TRADING_SCREEN);
    ...
    public static class GatecraftingScrollPanel extends ScrollPanel {
        ...
        public GatecraftingScrollPanel(Minecraft client, GatecraftingScreen screen,
                                       List<Recipe<CraftingContainer>> recipes, int left, int top, int width,
                                       int height) {
            ...
            if (world != null) {
                for (Recipe<CraftingContainer> recipe :
                        RecipeRegistrar.getAllGatecraftingRecipes(world.getRecipeManager())) {
                    RecipeButton recipeButton = new RecipeButton(screen, recipe, left, top + totalButtonHeight,
                            buttonWidth);
