...
public class GatecraftingMenu extends AbstractContainerMenu
{
	...
	public static GatecraftingMenu getClientContainer(int id, Inventory playerInventory)
	{
		...
		return new GatecraftingMenu(id, playerInventory, BlockPos.ZERO);
	}
...
//		this.addSlot(new GatecraftingResultSlot(this, playerInventory.player, this.craftingInventory, this.craftResult, OUTPUT_SLOT_ID, inputOffsetX + 94, inputOffsetY + slotHeight));
	...
	{
		Optional<Recipe<CraftingContainer>> filteredRecipe = recipeHolder.filter(recipe -> GatecraftingRecipe.doesPlayerHaveIngredients(this.player.getInventory(), recipe));
